...
abstract class Api {
  ...
  static getWarehouses({Manager? manager}) async {
    if (Manager == null) {
      return;
    }
    Completer completer = Completer();
    ...
    if (json.containsKey('warehouses')) {
      json.forEach((key, value) {
        if (key == 'warehouses') {
          final listWarehouses = value as List;
          warehouses =
              List.from(listWarehouses.map((e) => Warehouse.fromJson(e)));
        }
      });
    }
    return warehouses;
